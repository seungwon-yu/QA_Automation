...
| `tests/e2e/envFailEvidence.spec.js` | ENV_FAIL 분류 검증용 의도된 실패 샘플 |
...
`npm run test:e2e:env-fail-evidence`는 `TC-008-07` 기준 ENV_FAIL 분류 흐름을 검증하기 위한 의도된 실패 명령이다. 테스트 대상 서버 연결 실패를 제품 결함으로 단정하지 않고 실행 환경 문제로 분류하는지 확인한다.
